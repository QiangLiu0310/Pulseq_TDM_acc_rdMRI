...
seq_file='signleecho_mb1_18sli_2p5mm_R3_multib_te4_b.seq';
% Set system limits
B0field=2.89;% 6.98
...
tmp=load('/rfanfs/pnl-zorro/home/ql087/qiang_gSlider_data/lq/Pulseq_Qiang_PNL/matlab/demoSeq/diffusion_table/TMD_part2_bvec.txt');
tmp=tmp(3:end,:);
tmp(:,3)=-tmp(:,3);
...
crusher_switch=0;
bFactor=[500:1000:3000]'.*ones(1,size(table,1)); % s/mm^2
bFactor(:,1:3)=0;
...
for b_value=1:size(bFactor,1)
    for ds=1:dummy_count
        ...
        for Nmulti=1:RSegment
            ...
            %% TR calculation
            TR=2500e-3;
            delayTR=TR/Nslices*2-mr.calcDuration(gp_r) - mr.calcDuration(gn_r) - TE - rfCenterInclDelay-dur2end;
            dTE_incre=mr.makeDelay(TE_incre);
            delayTR=delayTR-2*TE_incre;
...
%             for s=1:Nslices/2 % Nslices/2+1:Nslices
      for s=Nslices/2+1:Nslices
