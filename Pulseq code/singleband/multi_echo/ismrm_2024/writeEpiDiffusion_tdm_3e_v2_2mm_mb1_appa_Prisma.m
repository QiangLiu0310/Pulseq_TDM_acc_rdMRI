...
field=2.89; % prisma 2.89
seq_name='tdm_3e_mb1_21sli_2mm_R3_1b_1dir_10ave_0905.seq';
...
est_flat=600e-6; %duration 600 us
...
for tdmloop=1:1%tdm
    ...
    for ds=1:dummy_count % actually it is diffusion count
        ...
        readoutTime=4.8e-4;        % this controls the readout bandwidth  2000/0.58 ms % 3.9
        ...
        [rf1, gz1, gzReph,gzReph_double] = mr.makeSincPulse_QL_1_tdm(pi/2,'system',lims1,'Duration',tRFex,...
        ...
        dur_rephase=mr.calcDuration(gzReph);
        dphase=mr.makeDelay(dur_rephase);
        ...
        gyPre = mr.makeTrapezoid('y',lims,'Area',Ny_pre*deltaky);
        ...
        % duration of EPI mod
%         epi_dur=mr.calcDuration(gx)*Ny_meas+mr.calcDuration(gxPre_post)+mr.calcDuration(gyPre);
epi_dur=mr.calcDuration(gx)*Ny_meas+mr.calcDuration(gxPre_post,gyPre_post)+mr.calcDuration(gyPre); 
        ...
        if (ds < 5) % need to adjust delta TE1 and delta TE2 for b0, bcuz of the crusher
            crusher_switch=1;
        else
            crusher_switch=0;
        end
        ...
        if ((sum(table(ds,:))==0)||(sum(table(ds,:))==1)) % b=0 or dwi with diffusion gradient on one axis we keep using the older version
            ...
            if size(Gr,2)==3
                ...
                if size(Gr,2)==2
                    diffusion_blank=find( table(ds,:)==0);
        ...
        TR=4000e-3;
        Nsli=Nslices/tdm;
        delayTR=ceil((TR-Nsli*mr.calcDuration(rf_fs) - Nsli*mr.calcDuration(gz_fs)...
