...
seq_file='epidiff_3_shot_ref_2p5mm_10sli_appa_recon.seq';
% Set system limits
...
for ref=1:ref_count % ref 1 works as the dummy scan, and I also turn off the PE gradients to collect the reference
    ...
    for Nmulti=1:RSegment
        ...
        % TEShift=[0+10e-4 actual_esp./RSegment+10e-4]; % for two shot version
        % TEShift_c=actual_esp-TEShift; % for two shot version match the total time
        % dETS=mr.makeDelay(TEShift(Nmulti));
        % dETSC=mr.makeDelay(TEShift_c(Nmulti));
        % QL: somehow Pulseq do not allow us to insert a specific delay...
        % and also you cannot insert 0 as a delay...
        % let us just try another way:
        ...
        TR=3000e-3; % ql follow Congyu's sequence /s
