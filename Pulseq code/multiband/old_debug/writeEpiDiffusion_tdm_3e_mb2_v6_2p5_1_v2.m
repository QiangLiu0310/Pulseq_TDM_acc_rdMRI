...
%% multiband version Feb/8/2023

clear all;close all;clc
seq_name='tdm_3e_mb2_60sli_2p5mm_R3_multib_te123_v2_full.seq';
...
% Bandsep = [0 Nslices/mb]- Nslices/mb/2;% not sure
Bandsep=[0 30];
nBand   = 0;
rf1_90 = mrz.makeMBPulse_less(rf1.signal,'n_bands',nBand,'timeBwProduct',4,'band_sep',Bandsep);
rf1.signal=rf1_90;
...
Nslices=Nslices/mb;
% freqOffset_factor=freqOffset_factor(Nslices/2+1:Nslices*3/2); % ori
% freqOffset_factor=freqOffset_factor(Nslices+1:Nslices*2);
freqOffset_factor=freqOffset_factor(1:Nslices);
...
for b_value=1:size(bFactor,1)
    for tdmloop=1:tdm
        ...
        for ds=1:dummy_count % actually it is diffusion count
            ...
            readoutTime=4.3e-4;        % this controls the readout bandwidth  2000/0.58 ms
            ...
            % delayTE1=delayTE1-dur_rephase; %forgot for the first test, Jan 14
            ...
            TR=2500e-3; % ql follow Congyu's sequence /s
            Nsli=Nslices/tdm;
            ...
            for s=1:tdm:Nslices
                ...
                seq.addBlock(gxPre_post,gyPre_post,gzBlip_post);
                seq.addBlock(dTR); %QL test 0807 From Lipeng
            end%slice
        end %REF
    end % tdm different TE loop
end % b-value loop
...
seq.setDefinition('FOV', [fov fov thickness*Nslices*tdm]); % Qiang Liu modified for tdm+mb
