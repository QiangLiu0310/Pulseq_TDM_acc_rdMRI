...
ascName='/rfanfs/pnl-zorro/home/ql087/qiang_gSlider_data/MP_GradSys_P034_c_CX600.asc'; % 3T prisma
...
aflAcousticResonanceFrequency=[575 1060 1410 1920]
aflAcousticResonanceBandwidth=[70 70 80 120]
if ischar(ascName)
    hold on; 
    for i=1:length(aflAcousticResonanceFrequency)
        if aflAcousticResonanceFrequency(i)>0 %ascData.asGPAParameters.sGCParameters.
            xline(aflAcousticResonanceFrequency(i),'-');
            xline(aflAcousticResonanceFrequency(i)-aflAcousticResonanceBandwidth(i)/2,'--');
            xline(aflAcousticResonanceFrequency(i)+aflAcousticResonanceBandwidth(i)/2,'--');
        end
    end
end
