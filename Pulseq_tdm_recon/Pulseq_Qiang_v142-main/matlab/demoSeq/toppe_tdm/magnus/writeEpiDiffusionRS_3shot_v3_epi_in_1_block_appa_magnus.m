...
% thickness1=0.00125;            % slice thinckness
Nslices=21;
bFactor=0; % s/mm^2
TE=30e-3; % min is 65
...
ref_count=4; % QL set for reference scan
...
for ref=1:ref_count % ref 1 works as the dummy scan, and I also turn off the PE gradients to collect the reference
    ...
    for Nmulti=1:RSegment
        ...
        crusher_d=0.95e-3;
        % gz180_crusher=mr.makeTrapezoid('z',lims,'Area',-1*gzReph.area,'Duration',crusher_d);
        ...
        gy=trap4ge(gy,commonRasterTime,lims);% QL for GE
        ...
        adc = mr.makeAdc(adcSamples,'system', lims, 'Dwell',adcDwell,'Delay',blip_dur/2);% QL for GE
        ...
        %% for after the first echo QL, VERY IMPORTANT!

        gxPre_post = mr.makeTrapezoid('x',lims1,'Area',-gx.area/2);
        %         gyPre_post = mr.makeTrapezoid('y',lims1,'Area',Ny_post*deltaky);
        gyPre_post = mr.makeTrapezoid('y',lims1,'Area',(Ny_post*deltaky - (4 - Nmulti)*deltak));
        ...
        %  TEShift=2.30e-04; % it should be 2.3250e-04, but did not fit the time raster?
        ...
        TR=4000e-3; % ql follow Congyu's sequence /s
        ...
        for s=1:Nslices
            if (Nmulti==1)&&(ref==1)&&(s==1)
                seq.addBlock(gp_r,gp_p,gp_s, mr.makeLabel('SET', 'TRID', 1));
            else
            ...
            % ---------------------EPI in one block from Jon and Maxim-------------------
            seq_epi_d = mr.Sequence(lims);   % dummy sequence object for EPI readout

            for i=1:Ny_meas
                if i==1
                    seq_epi_d.addBlock(gx,gy_blipup,adc); % Read the first line of k-space with a single half-blip at the end
                elseif i==Ny_meas
                    seq_epi_d.addBlock(gx,gy_blipdown,adc); % Read the last line of k-space with a single half-blip at the beginning
                else
                    seq_epi_d.addBlock(gx,gy_blipdownup,adc); % Read an intermediate line of k-space with a half-blip at the beginning and a half-blip at the end
                end
                %                     gx = mr.scaleGrad(gx,-1); % the next line will result in a wrong kx trajectory for Ny_meas=odd, QL, Oct 20
            ...
            wave_data = seq_epi_d.waveforms_and_times();
            wave_length = seq_epi_d.duration/lims.gradRasterTime;
            wave_time = ((1:wave_length)-0.5)*lims.gradRasterTime; %-0.5
            ...
            gx_traj = mr.makeArbitraryGrad('x', wave_x, lims,'delay',lims.adcDeadTime);
            gy_traj = mr.makeArbitraryGrad('y', wave_y, lims,'delay',lims.adcDeadTime);

            % make new ADC
            adcDur=seq_epi_d.duration-2*lims.adcDeadTime+0.3*mr.calcDuration(gxPre_post); % dead times at the beginning and at the end
            adcSamplesPerSegment=300; % we need some "roundish" number of samples 300
            adcNumSam=floor(adcDur/adc.dwell/adcSamplesPerSegment)*adcSamplesPerSegment;
            adc_traj=mr.makeAdc(adcNumSam,'dwell',adc.dwell,'system',lims);

            % define real sequence blocks
            % if (ref ==1)
            %     seq.addBlock(gx_traj,gy_traj);
            % else
