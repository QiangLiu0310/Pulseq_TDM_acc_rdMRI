% line 229: add a TRID label
% use Pulseq_v_142
% GE MAGNUS
% from Jon: assign a TRID to each Nmulti
clc;close all;clear all;
seq_file='epidiff_3_shot_ref_2mm_21sli.seq';
% Set system limits
lims = mr.opts('MaxGrad',50,'GradUnit','mT/m',...
    'MaxSlew',740,'SlewUnit','T/m/s',...
    'rfRingdownTime', 10e-6, 'rfDeadtime', 100e-6, 'B0', 3); % 50 740
lims1 = mr.opts('MaxGrad',100,'GradUnit','mT/m',...
    'MaxSlew',100,'SlewUnit','T/m/s',...
    'rfRingdownTime', 10e-6, 'rfDeadtime', 100e-6, 'B0', 3);  % gz fat

%%  GE  system QL
% when designing: 40 us for
...
[rf, gz, gzReph] = mr.makeSincPulse(pi/2,'system',lims1,'Duration',tRFex,...
...
[rf180, gz180] = mr.makeSincPulse_QL_1(pi,'system',lims,'Duration',tRFref,...
...
gx = mr.makeTrapezoid('x',lims,'Area',kWidth+extra_area,'duration',readoutTime+blip_dur); %QL: for GE
...
adc = mr.makeAdc(adcSamples,'system', lims, 'Dwell',adcDwell,'Delay',blip_dur/2); % QL add a sys option Apr 29
...
gyPre = mr.makeTrapezoid('y',lims,'Area',(Ny_pre*deltaky));% QL I believe it should be deltak instead of deltaky
...
for ref=1:ref_count % ref 1 works as the dummy scan, and I also turn off the PE gradients to collect the reference
    ...
    for Nmulti=1:RSegment
        ...
        % This depends on Nmulti
        actual_esp=gx.riseTime+gx.flatTime+gx.fallTime;
        TEShift=actual_esp/RSegment;
        TEShift=round(TEShift,5); % from Berkin: roundn didn't work for the latest matlab, changed to round (sign -/+)
        ...
        for s=1:Nslices
            % if (Nmulti==1)&&(ref==1)&&(s==1) % each of Nmulti will need a TRID  TRID=Nmulti
            ...
            scale_gyPre_segment = (Ny_pre*deltaky-(Nmulti-1)*deltak) / (Ny_pre*deltaky);
            seq.addBlock(gxPre, mr.scaleGrad(gyPre, pe_enable*scale_gyPre*scale_gyPre_segment));
            for i=1:Ny_meas
                if i==1
