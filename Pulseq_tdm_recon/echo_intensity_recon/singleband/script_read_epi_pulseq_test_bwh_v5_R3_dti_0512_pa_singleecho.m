...
%%
% section a
data_path = '/data/pnlx/home/ln915/Data/0613_phantom/'
filename1 = 'meas_MID00026_FID78124_SE3_Sinc4'; % let's read the reference first.
D=dir([data_path filename1]);
[~,I]=sort([D(:).datenum]);
twix_obj = mapVBVD([data_path filename1]);
seq = mr.Sequence();
read(seq,'/rfanfs/pnl-zorro/home/ql087/qiang_gSlider_data/lq/Tests/Test_77_May_11/recon/signleecho_mb1_10sli_2p5mm_R3_te1_slr_recon.seq')
...
tmp=[1 3];
rawdata=reshape(rawdata,[212 32 22 10 5]);
rawdata=squeeze(rawdata(:,:,:,:,tmp));
rawdata=reshape(rawdata,[212 32 440]);
% rawdata=squeeze(rawdata(:,:,:,1,:,tmp));
% rawdata=reshape(rawdata,[212 32 440]); clear tmp;
...
ktraj_adc2=reshape(ktraj_adc2,[size(ktraj_adc2,1) size(ktraj_adc2,2) 1100 1]); % size(ktraj_adc2,3)/Rseg/diff_plus_dum Rseg*diff_plus_dum
...
sz_k=[Nx nCoils Ny_sampled slice_num ]; %I treat the gSlider loop as the diffusion
Kimage_first = reshape(k_gc,sz_k);
Kimage_first_full=zeros(Nx, nCoils, Ny_sampled*3,slice_num);
Kimage_first_full(:,:,3:3:end,1:slice_num) = Kimage_first;

for islice_1=1:slice_num
    freqOffset_factor(islice_1)=islice_1-1-(slice_num-1)/2;
end

slic_indexS=[1:slice_num];
slic_indexS=reshape(slic_indexS,[slice_num/2, 2])';
slic_indexS=reshape(slic_indexS,[slice_num,1]);
interl=freqOffset_factor(slic_indexS);
[~,imgor]=sort(interl);
Kimage_first_full=Kimage_first_full(:,:,:,imgor);



%% ACS data
data_path = '/data/pnlx/home/ln915/Data/0613_phantom/'
filename1 = 'meas_MID00022_FID78120_threeshot_ref'; % let's read the reference first.
D=dir([data_path filename1]);
[~,I]=sort([D(:).datenum]);
twix_obj = mapVBVD([data_path filename1]);
seq = mr.Sequence();
read(seq,'/rfanfs/pnl-zorro/home/ql087/qiang_gSlider_data/lq/Tests/Test_77_May_11/recon/epidiff_3_shot_ref_2p5mm_10sli_appa_recon.seq')
[ktraj_adc, t_adc, ktraj, t_ktraj, t_excitation, t_refocusing] = seq.calculateKspacePP();
...
save_path='/rfanfs/pnl-zorro/home/ql087/qiang_gSlider_data/lq/data_processing/TDM_revision/phantom/me_sinc/';
save([save_path 'se_te3_sinc4_pa.mat'], 'I_short', '-v7.3')
