...
filename1 = 'meas_MID00045_FID78143_ME_Sinc8'; % let's read the reference first.
...
rawdata=reshape(rawdata,[212 32 22 3 10 5]);
rawdata=permute(rawdata,[1 2 3 5 6 4]);
rawdata=reshape(rawdata,[212,32,22*3*10*5]);
...
%% ACS data
data_path = '/data/pnlx/home/ln915/Data/0613_phantom/'
filename1 = 'meas_MID00046_FID78144_threeshot_ref'; % let's read the reference first.
...
I_short=reshape(I_short, [88 88 10 14]);
tmp=zeros(88,88,10,15);
tmp(:,:,:,2:end)=I_short;
tmp=reshape(tmp,[88 88 10 5 3]);
tmp1=[2 4:5];
tmp=tmp(:,:,:,tmp1,:,:);
I_short=tmp;
clear tmp
...
save([save_path 'phantom_me_sinc8_te123.mat'], 'I_short', '-v7.3')
