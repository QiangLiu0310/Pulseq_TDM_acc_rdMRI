...
%**************************************************************************%
  ...
  FLAG__siemens_style = 0;
  if ( nargin >= 4 ),
    FLAG__siemens_style = varargin{2};
  end;
  ...
  if ( FLAG__siemens_style ),
    ft = fftshift( fft(ifftshift(raw, dim), Npoint, dim), dim);
  else,
    ft = ifftshift(raw, dim);
    ft = ifft(ft, Npoint, dim);
    ft = fftshift(ft, dim) * Npoint;
  end;
