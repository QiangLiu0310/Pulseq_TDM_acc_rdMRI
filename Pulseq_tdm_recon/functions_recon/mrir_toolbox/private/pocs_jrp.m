function [im, kspFull] = pocs_jrp( ksp, iter, watchProgress )
...
% jonathan polimeni <jonp@nmr.mgh.harvard.edu>, 2015/apr/18
% [[ edited only to make output less verbose ("fprintf_jrp") ]]
    ...
    if nnz(smplPtrn) == numel(smplPtrn)     % only the sampled data were passed / |N|umber of |N|on |Z|ero elements
	  ...
	  centreLine = size( smplPtrn, pfDim) / 2;
    ...
    [ centreLine, idxSym ] = findSymSampled( ksp, pfDim, Nsmp, centreLine );

    szSym = numel( idxSym );                % 2 * (Nsmp - centreLine) + 1

    if isUpper
        fprintf_jrp('Using %g points around point %g\n', szSym,    centreLine    );
    else
    ...
    for ii = double(~watchProgress) : iter
        ...
        if ii > 0
            ...
            fprintf_jrp([repmat('\b',1,prevLength) '%s' '%s'], progress_str, edging );
    ...
    fprintf_jrp([repmat('\b',1, numel(progress_str) + numel(edging)) 'POCS done! (%g s)' '%s\n\n'], t, edging );
    % ) ===================================================================
...
function [ ksp, pfDim, isUpper, isLower, Nsmp ] = zerofillPFdim( ksp, wasAddedCoilDim )
    ...
    switch nnz(isPartialFourier)    % |N|umber of |N|on |Z|ero elements
        ...
        case 1
            fprintf_jrp( 'Found partial Fourier along array dimension %d\n', pfDim + ~wasAddedCoilDim )
        otherwise
...
function [ centreLine, idxSym ] = findSymSampled( ksp, pfDim, Nsmp, centreLine )
