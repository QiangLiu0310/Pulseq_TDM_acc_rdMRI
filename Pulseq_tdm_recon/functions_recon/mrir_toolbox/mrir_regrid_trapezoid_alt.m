function varargout = mrir_regrid_trapezoid(raw, prot, varargin)
...
%**************************************************************************%
  ...
  if ( isempty(prot_trapezoid) ),
    ...
    prot_trapezoid = mrir_regrid_trapezoid_prep(prot, size(raw, 1));
  end;
  ...
  if ( DO__DEAPOD ),
    ...
    % project data into image space for deapodization
    img_regrid = mrir_iDFT_freqencode(raw_regrid);
    
    img_deapod = mrir_regrid_trapezoid_rolloff(img_regrid, prot_trapezoid);
  
    % project image back into k space for further processing
    raw_deapod = mrir_fDFT_freqencode(img_deapod);

    varargout{1} = raw_deapod;
    
    
  else,
%    disp('skipping deapodization');
    varargout{1} = raw_regrid;
  end;
