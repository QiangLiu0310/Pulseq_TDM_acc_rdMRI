...
% $Id: mrir_fDFT.m,v 1.1 2009/01/30 06:09:47 jonp Exp $
%**************************************************************************%
  ...
  if ( Npoint > size(img, dim) ),
    ...
    img = mrir_zeropad(img, pad_dims, 'post');

  end;

  ksp = fftshift(fft(ifftshift(img, dim), Npoint, dim), dim) / Npoint;
  ...
  %************************************************************************%
  %%% $Source: /space/padkeemao/1/users/jonp/cvsjrp/PROJECTS/IMAGE_RECON/mrir_toolbox/mrir_fDFT.m,v $
  %%% Local Variables:
  %%% mode: Matlab
  %%% fill-column: 76
  %%% comment-column: 0
