function [A] = recon_sms_mp2slice( varargin )
...
%
NMultiplex = 2;%  sequence mp2slice;
hdr = parse_measdat_hdr(k.image.filename);
%hdr = parse_measdat_hdr_Yang(k.image.filename);
...
for cmux=1:NMultiplex
    %--------------------------------------------------------------------
    index_mux = mod(cmux-1,NMultiplex)+1;
...
for cntsli=1:size(aa,3)
    ...
    img_part1(:,:,cntsli,:) =sqrt(sum(abs( fif( img_aa ) ).^2,3));
    img_part2(:,:,cntsli,:) =sqrt(sum(abs( fif( img_bb ) ).^2,3));

end

[s1, s2, s3, s4]=size(img_part1);
A=zeros([s1 s2 2*s3 s4]);

A(:,:,1:NSlc,:)        = img_part1;
A(:,:,NSlc+1:2*NSlc,:) = img_part2;
