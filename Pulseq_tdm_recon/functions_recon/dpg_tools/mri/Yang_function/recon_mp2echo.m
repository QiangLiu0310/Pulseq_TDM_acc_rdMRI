function [A,B] = recon_mp2echo( varargin )
...
NSlc = hdr.NSlc;
R = hdr.R;
NEcho = 2;% two echo sequence mp2echo;
...
% refscanPC dimension:
% 'NFreq_outres', 'Ncoils', '2(RO+ and RO-)','2(two Echo)','NSlc', 'NRep'
...
vrgf_refscanPC_first   = squeeze(vrgf_refscanPC_ROp_ROn(:,:,:,2,reordr_S,:));
vrgf_refscan_first     = squeeze(vrgf_refscan(:,:,:,1,reordr_S,:));
vrgf_phasecor_first    = squeeze(vrgf_phasecor_ROp_ROn(:,:,:,2,reordr_S,:));
...
vrgf_refscanPC_second  = squeeze(vrgf_refscanPC_ROp_ROn(:,:,:,1,reordr_M,:));
vrgf_refscan_second    = squeeze(vrgf_refscan(:,:,:,2,reordr_M,:));
vrgf_phasecor_second   = squeeze(vrgf_phasecor_ROp_ROn(:,:,:,1,reordr_M,:));
...
if (nargin > 2)  %if R=2, internal is OK, but if R>3, internal ACS data is bad, we need use external ACS!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    [~,Ng_external] = recon_std_EPI(varargin{2});
%     Ng_first = Ng_external(1:NSlc);
%     Ng_second = Ng_external(NSlc+1:2*NSlc);
