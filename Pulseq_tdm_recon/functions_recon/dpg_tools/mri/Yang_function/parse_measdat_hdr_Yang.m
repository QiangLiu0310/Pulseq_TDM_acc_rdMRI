...
if strcmp(data_version,'VD');
    ...
    tmp = '<ParamLong."SMSAccFactor">  {';
    indx = strfind(asc,tmp);
    if ~isempty(indx)
        hdr.SMS = sscanf( asc( indx(1)+length(tmp)+(0:10) ), '%d' );
        %   else
        %     hdr.SMS = 1;
    end;
...
tmp = '<ParamLong."NColMeas">  {';
indx = strfind(asc,tmp);
if ~isempty(indx)
    hdr.NColMeas = sscanf( asc( indx(1)+length(tmp)+(0:10) ), '%d' );
end;

tmp = '<ParamLong."NSegMeas">  {';
indx = strfind(asc,tmp);
if ~isempty(indx)
    hdr.NSegMeas = sscanf( asc( indx(1)+length(tmp)+(0:10) ), '%d' );
end;
