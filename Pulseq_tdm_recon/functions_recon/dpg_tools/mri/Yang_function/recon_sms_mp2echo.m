function [img_short, img_long] = recon_sms_mp2echo( varargin )
%% add function:SEloop_mode 20200809
%  -------------------------------------------------------------------
% |         SEloop_mode:0           |          SEloop_mode:1        |     
% |                                 |                               |       
% |    ShortEcho   |   LongEcho     |     LongEcho     | ShortEcho  |  
% |      Part-I    |   Part-II      |     Part-II      |   Part-I   | 
%  -------------------------------------------------------------------
twix = varargin{1};
if iscell(twix)
    k = twix{length(twix)};
...
%
NMultiplex = 2;% two echo sequence mp2echo;
...
%%%%%%%Caution here%%%%%%%%%%%%%%%%%%
slic_index_MB = [2:2:NSlc/hdr.SMS 1:2:NSlc/hdr.SMS];%????????Guess it depends on interleaved ascending or Descending?????????
...
Kimage_long   =squeeze(Kimage(:,:,:,2,order_MB,:));% long
...
for cmux=1:NMultiplex
    %--------------------------------------------------------------------
    index_mux = mod(SEloop_mode+cmux-1,NMultiplex)+1;
...
for cntsli=1:size(aa,3)
    ...
    for cntcoil=1:size(aa,4)
        for cRep=1:size(aa,5)
            ...
            img_bb(:,:,cntcoil,cRep)=flip(reconhd(bb(:,:,cntsli,cntcoil,cRep),size(bb,1),size(bb,2)));    
    ...
    img_long (:,:,cntsli,:) =sqrt(sum(abs( fif( img_bb ) ).^2,3));
